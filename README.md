...
[![Version](https://img.shields.io/badge/version-1.0.53-blue)](https://github.com/APecme/Telegram_Depiler/releases/tag/v1.0.53)
...
- `{timestamp}` - 时间戳
- `{file_name}` - 原始文件名
- `{message_text}` - 消息文字或媒体说明

所有变量均可使用 `{变量:最大字符数}` 限制长度，例如 `{message_text:30}` 会保留消息文字的前 30 个字符。
